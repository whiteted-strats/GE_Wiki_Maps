...
def streets_specific(tilePlanes, currentTiles, plt, axs):
    ...
    0b,13,12,06,05,04,03 : 002d (outside of 2nd left corner)    (*)
        avoid these 2 with death anim
        OR get the 1st one but be deleted by the 80m. It's tight though so he'd have to not move too far forward,
          and probably animate with good timing to then unload back to his original pad
    14,15,16 : 0036 (outside of the corner before kf7 guards. Can't spawn until some way into 14) (bad surely)
    17,18,19 : 004c (after between-the-cars -> well into the right branch) (bad)
        => maybe left strafe to prevent it as a procaution could be sensible.
        => leave Val in right then switch at these kills? Left would have a prettier view :)
    ...
    25,26,27 : 0076 (long straight -> just around the corner) (very start of 25 isn't in range) (*)
    ...
    for r in roomPath:
        ...
        allBlueSpawnLocs.add(blueSpawnPad)
        ##print(f"{r:02x} : {blueSpawnPad:04x}")
    ##print("")

    """
    BLUE (running guard, ID 0x22, 9s timer)
        Both work fine with RED
    0b,13,12,06,05,04,03 : 0031 (behind left corner) *
    14,15,16 : 003e (111 1st boost) (* - death finished by the end of 16, timing would be semi precise)
    17,18,19 : 0046 (outside of next corner) (bad but we'll avoid it)
    1a,1b,24 : 006d (mid of long straight, bad on console on agent..) (*)
        => should path inwards
    ...
    # Civilian of interest (8s, 0x2d) is gonna be in green
    # But we see we just specified pads with this scripts so we do that

    
    if True:
        chosenSpawnLocs = [
            ([0x0031, 0x003e, 0x006d, 0x0082], 'b'),    # BLUE
            ([0x002d, 0x0069, 0x0076, 0x0085], 'r'),    # RED
            ([0x0030], 'g'),                            # SURPRISE
        ]
    else:
        chosenSpawnLocs = [
            (list(allBlueSpawnLocs), 'b'),
            (list(allRedSpawnLocs), 'r'),
        ]

    
    STD_SPEED = 5.4807696015788

    # Gromic would prefer the maps a little cleaner
    """
    for sps, colour in chosenSpawnLocs:
        for sp in sps:
            x,_,z = pads[sp]["position"]
            axs.add_artist(plt.Circle((-x, z), 8000 + 800, color=colour, alpha=0.5, linewidth=1, fill=False))
            ...
            drawPathWithinGroup(plt, axs, spawnPath[-3:], pads, currentTiles, tiles, None, stdColour=colour)
    """
    ...
    # And a little calculation for our civilian
    p = getPathBetweenPads(0x0030, padNearVal, sets, pads)[:2]
    assert p[1] == 0x0031
    t = getPathTime(None, p, pads, STD_SPEED) / 60
    print(f"Civ -> Guard spawn: {t}")
